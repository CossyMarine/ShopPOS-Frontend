...
import { X, Banknote, Smartphone, Landmark, Gift, CheckCircle2, Printer } from 'lucide-react';
...
export default function PaymentModal({ receipt, onClose, onComplete }) {
    ...
    const handleCash = async () => {
        ...
        if (isNaN(received) || received < balanceDue) return toast.error('Amount received is less than the balance due');
        setProcessing(true);
        try {
            await API.patch(`/receipts/${receipt._id}/pay`, { amountPaid: received });
            const change = received - balanceDue;
            if (change > 0) toast.info(`Give KES ${change.toLocaleString()} change`);
            finishAndAskReward();
        } catch (err) {
            ...
            <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-200">
                ...
                {method === 'cash' && (
                    <div className="space-y-3">
                        <input type="number" autoFocus value={amountReceived} onChange={(e) => setAmountReceived(e.target.value)}
